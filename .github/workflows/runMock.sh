...
# Verify installation
~/.speedscale/proxymock version || { echo "Proxymock installation failed"; exit 1; }
...
nohup ~/.speedscale/proxymock run --service http=18080 --service https=18443 --snapshot-id "$SNAPSHOT_ID" > proxymock.log 2>&1 &
